...
def get_user_ingredients(): 
    ...
    entered_ingredients = input(
        "\nPlease list ingredients you're starting with, separated by a comma: ")
...
def retrieve_best_recipe(api_key,target_ingredients):
    ...
    # or an error occurs
    search_for = {"key": api_key, "q": ",".join(target_ingredients), "sort": "r"}
    
    try:
        recipe_list = get(RECIPE_SEARCH_URL, params=search_for,timeout=5)         
    except:
        # handle site down or internet down errors        
        print("\nLooks like the recipe server is unavailable. Either the site is down\
         or you don't have an internet connection\n")       
        return
    else:
        if recipe_list.status_code == 200:                                 
            ...
            # Checks that site has returned recipes rather than an error
                if recipe_list.json()["count"] > 0:
                    # if recipes were found, retrieve recipe_id of first recipe on list 
                    best_recipe = recipe_list.json()["recipes"][0]["recipe_id"] 
                    return (best_recipe)                 
                else:    
                    print ("\nThere were no recipes that included all your ingredients\n")
                    return 
            else:          
                print ("\nYou have exceeded your daily maximum number of queries\nGoodbye")      
                quit()
        else: 
...
def retrieve_recipe_ingredients(api_key, recipe_id):
    ...
    search_for = {"key":api_key,"rId":recipe_id}
    try:
        recipe_detail = get(RECIPE_DETAIL_URL, params=search_for, timeout=5)  
    except:
        # handle site down or internet down errors
        print("\nLooks like the recipe server is unavailable. Maybe your internet is down\n")      
        return    

    if recipe_detail.status_code == 200:
        if list(recipe_detail.json().keys())[0] == 'recipe': 
            if recipe_detail.json() == []:
                # catches empty recipes.  Shouldn't happen
                print ("\nSorry, there were no recipes that included all of your ingredients\n")           
                return
            else:    
        ...
        # handle error codes from server      
        print("\nServer returned error {}\n".format(recipe_detail.raise_for_status ))  
        return
...
def recommend_recipe(recipe_name):
    ...
    print("{:<80}".format("\n\nWhat's Cooking today is {}, which is the most popular recipe "
...
def you_need_list(target_ingredients,recipe_ingredients):
    ...
    ingredients_needed = [ingredient for ingredient in recipe_ingredients 
                         if not any([search(
                             r"( |^|\,){}(s|es)?(\,|$| )".format(target_ingredient),
...
def you_may_need_list(target_ingredients,recipe_ingredients):    
    ...
    for target_ingredient in target_ingredients:
        ...
        ingredient_matches = ([ingredient for ingredient in recipe_ingredients 
                             if search(r"( |^|\,){}(s|es)?(\,|$| )".format(target_ingredient),
                             clean_text(ingredient))!=None])
        if len(ingredient_matches)>1: you_may_need.append(ingredient_matches)
        # If there is more than one ingredient match, add all matched ingredients to 
        # "You may need list"
    ...
    if you_may_need: 
        ...
        for i,duplicate_matches in enumerate(you_may_need):
            ...
            [print("{}  OR  ".format(duplicate),end=" ") for duplicate in duplicate_matches[:-1]]
            print("{}.".format(duplicate_matches[-1]))
...
while True:
    ...
    if best_recipe_id:
        ...
        best_recipe_name,all_ingredients = retrieve_recipe_ingredients(user_key,best_recipe_id)
